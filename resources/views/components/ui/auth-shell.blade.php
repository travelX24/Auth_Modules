...
<x-authkit::ui.auth-layout :title="$title">
    ...
    <div class="min-h-screen flex items-center justify-center p-3 sm:p-4 md:p-6 relative">
        ...
        <!-- Background gradient with pattern -->
            ...
            <div class="grid lg:grid-cols-2 relative z-10 h-full">
                ...
                <div class="relative p-6 sm:p-8 lg:p-10 xl:p-12 h-full hidden lg:block">
                    ...
                    <div class="absolute top-8 {{ $isRtl ? 'right-8' : 'left-8' }} w-24 h-24 sm:w-28 sm:h-28 rounded-full bg-gradient-to-r from-[color:var(--brand-from)] to-[color:var(--brand-to)] opacity-20 blur-2xl animate-pulse-glow"></div>
        ...
        <div class="relative h-48 sm:h-56 mb-8">
            <!-- Main HR Dashboard Card -->
            <div class="absolute {{ $isRtl ? 'right-4' : 'left-4' }} top-2 w-40 h-40 sm:w-44 sm:h-44 bg-gradient-to-br from-blue-500/25 to-cyan-500/25 rounded-2xl sm:rounded-3xl rotate-3 border-2 border-white/40 shadow-xl sm:shadow-2xl backdrop-blur-lg overflow-hidden">
                <!-- Dashboard Grid -->
                <div class="absolute inset-2 grid grid-cols-3 grid-rows-3 gap-1.5">
                    @foreach(range(1, 9) as $i)
                    <div class="bg-white/20 rounded-md {{ $i % 3 === 0 ? 'animate-pulse' : '' }}"></div>
                    @endforeach
                </div>
                <!-- HR Badge -->
                <div class="absolute top-3 {{ $isRtl ? 'left-3' : 'right-3' }} w-8 h-8 bg-gradient-to-r from-emerald-400 to-blue-400 rounded-lg flex items-center justify-center shadow-md">
                    <span class="text-white font-bold text-[10px]">HR</span>
                </div>
            </div>
            
            <!-- Employee Profile Card -->
            <div class="absolute {{ $isRtl ? 'right-12' : 'left-12' }} top-12 w-36 h-36 sm:w-40 sm:h-40 bg-gradient-to-br from-purple-500/25 to-pink-500/25 rounded-2xl sm:rounded-3xl -rotate-6 border-2 border-white/40 shadow-xl sm:shadow-2xl backdrop-blur-lg animate-float-slow overflow-hidden">
                <!-- Profile Elements -->
                <div class="absolute top-5 inset-x-5 h-8 bg-white/30 rounded-full"></div>
                <div class="absolute top-16 inset-x-6 h-3 bg-white/20 rounded-full"></div>
                <div class="absolute top-22 inset-x-8 h-3 bg-white/20 rounded-full"></div>
            </div>
            
            <!-- Calendar/Time Card -->
            <div class="absolute {{ $isRtl ? 'right-20' : 'left-20' }} top-20 w-32 h-32 sm:w-36 sm:h-36 bg-gradient-to-br from-amber-500/20 to-orange-500/20 rounded-xl sm:rounded-2xl rotate-12 border-2 border-white/40 shadow-lg sm:shadow-xl backdrop-blur-md animate-float-delayed">
                <!-- Calendar Grid -->
                <div class="absolute top-3 inset-x-3 grid grid-cols-4 gap-1">
                    @foreach(range(1, 12) as $i)
                    <div class="h-1.5 bg-white/25 rounded"></div>
                    @endforeach
                </div>
                <!-- Time Display -->
                <div class="absolute bottom-5 inset-x-6 h-5 bg-gradient-to-r from-white/40 to-white/20 rounded-lg"></div>
            </div>
        </div>
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center gap-2.5">
                                <div class="h-7 w-7 rounded-full bg-gradient-to-r from-[color:var(--brand-from)] to-[color:var(--brand-to)] flex items-center justify-center flex-shrink-0">
                                    <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                </div>
                                <span class="text-sm sm:text-base text-slate-700 font-medium">@tr('Attendance & leave management')</span>
                            </div>
                            <div class="flex items-center gap-2.5">
                                <div class="h-7 w-7 rounded-full bg-gradient-to-r from-[color:var(--brand-from)] to-[color:var(--brand-to)] flex items-center justify-center flex-shrink-0">
                                    <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <span class="text-sm sm:text-base text-slate-700 font-medium">@tr('Payroll & allowances in one place')</span>
                            </div>
                            <div class="flex items-center gap-2.5">
                                <div class="h-7 w-7 rounded-full bg-gradient-to-r from-[color:var(--brand-from)] to-[color:var(--brand-to)] flex items-center justify-center flex-shrink-0">
                                    <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <span class="text-sm sm:text-base text-slate-700 font-medium">@tr('Employee records & approvals workflow')</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-6 sm:p-8 lg:p-10 xl:p-12 h-full flex flex-col">
                    ...
                    <div class="lg:hidden mb-6">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2.5">
                                <div class="h-9 w-9 rounded-lg bg-gradient-to-br from-[color:var(--brand-from)] to-[color:var(--brand-to)] flex items-center justify-center">
...
<div class="hidden lg:block mb-8 xl:mb-10">
    <div class="text-center">
        <h1 class="text-2xl sm:text-2.5xl xl:text-3xl font-bold sm:font-extrabold text-slate-900 leading-tight">
            @tr('Welcome back')
        </h1>
    ...
    <div class="mt-5 sm:mt-6 flex justify-center">
        <div class="inline-flex items-center gap-1.5 sm:gap-2 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl bg-white/70 backdrop-blur-md shadow-sm ring-1 ring-slate-200/70">
            <a href="{{ route('authkit.login') }}"
               class="px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold sm:font-bold rounded-lg sm:rounded-xl transition-all duration-200
               {{ $active === 'login'
