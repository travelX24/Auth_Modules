    ...
    <div class="space-y-4 sm:space-y-5">
        ...
        <div class="relative">
            ...
            <div class="relative flex justify-center">
                <span class="px-3 sm:px-4 bg-white text-xs sm:text-sm text-slate-500">@tr('Informations')</span>
            </div>
        </div>

        <!-- Login form -->
        <form method="POST" action="{{ route('authkit.login.store') }}" class="space-y-4 sm:space-y-5">
            @csrf
            ...
            <div class="mt-3 sm:mt-4 flex items-center justify-between">
                <label class="flex items-center gap-2 sm:gap-2.5 cursor-pointer group">
                    <div class="relative">
                    ...
                    <span class="text-xs sm:text-sm text-slate-600 group-hover:text-slate-900 transition-colors">@tr('Remember me')</span>
                </label>
            </div>

            <x-ui.primary-button class="mt-2 sm:mt-3" >
                @tr('Sign in')
            </x-ui.primary-button>
        </form>
    </div>
